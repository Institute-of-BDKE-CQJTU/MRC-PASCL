...
def mask_tokens(
    inputs: torch.Tensor, mlm_probability: float = 0.15
...
def get_contrastive_input(input_ids, start_positions, end_postions):
    questions_idx = torch.nonzero(input_ids == 102)
    questions_idx_new = questions_idx[::2, :]
    questions_idx = questions_idx_new.clone()
    questions_idx[:,0] = 1
    questions_idx[:, 1] = questions_idx[:, 1] - 1

    start_positions = start_positions.tolist()
    end_postions = end_postions.tolist()
    answers_idx = []
    negative_spans_index = []
    negative_spans_weight = []

    for i in range(len(start_positions)):
        temp_span_index = []
        temp_span_weight = []
        answers_idx.append([start_positions[i], end_postions[i]])
        if start_positions[i] == end_postions[i] and start_positions[i]== 0:
            for _ in range(20):
                temp_span_index.append([0, 0])
                temp_span_weight.append(0.0)
        else:
            for j in range(2, 7):
                if start_positions[i] - j > 0:
                    temp_span_index.append([start_positions[i] - j, start_positions[i] -1])
                    temp_span_weight.append(1.0)
                else:
                    temp_span_index.append([0, 0])
                    temp_span_weight.append(0.0)
            for j in range(2, 7):
                if end_postions[i] + j < 383:
                    temp_span_index.append([end_postions[i]+1, end_postions[i]+j])
                    temp_span_weight.append(1.0)
                else:
                    temp_span_index.append([0, 0])
                    temp_span_weight.append(0.0)
            for j in range(3, 8):
                if start_positions[i] - j > 0:
                    temp_span_index.append([start_positions[i] - j, start_positions[i] -2])
                    temp_span_weight.append(1.0)
                else:
                    temp_span_index.append([0, 0])
                    temp_span_weight.append(0.0)
            for j in range(3, 8):
                if end_postions[i] + j < 383:
                    temp_span_index.append([end_postions[i]+2, end_postions[i]+j])
                    temp_span_weight.append(1.0)
                else:
                    temp_span_index.append([0, 0])
                    temp_span_weight.append(0.0)
        # 一起构造
        negative_spans_index.append(temp_span_index.copy())
        negative_spans_weight.append(temp_span_weight.copy())
        temp_span_index.clear()
        temp_span_weight.clear()
    return negative_spans_index, negative_spans_weight, questions_idx, answers_idx
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.cl_span_loss == True:
                negative_spans_index, negative_spans_weight, questions_idx, answers_idx = get_contrastive_input(inputs['input_ids'].clone().cpu(), inputs['start_positions'].clone().cpu(), inputs['end_positions'].clone().cpu())
                inputs['negative_spans_index'] = torch.tensor(negative_spans_index, dtype=torch.long).to(args.device)
                inputs['negative_spans_weight'] = torch.tensor(negative_spans_weight, dtype=torch.float).to(args.device)
                inputs['questions_idx'] = torch.tensor(questions_idx, dtype=torch.long).to(args.device)
                inputs['answers_idx'] = torch.tensor(answers_idx, dtype=torch.long).to(args.device)
...
def main():
    ...
    parser.add_argument("--mlm_loss", default=False, type=str2bool, help="Whether to add mlm loss")
    parser.add_argument("--cl_span_loss", default=False, type=str2bool, help="Whether to add mlm loss")
    ...
    parser.add_argument("--lambda_value", type=int, default=2, help="lambda hyperparameter's value")
